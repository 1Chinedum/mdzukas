document.addEventListener('DOMContentLoaded', function() {
    ...
    const prefsPanel = document.getElementById('cookie-preferences');
    const chkAnalytics = document.getElementById('pref-analytics');
    const chkMarketing = document.getElementById('pref-marketing');
    ...
    if (btnAccept) {
        btnAccept.addEventListener('click', () => {
            setConsent({ consent: true, analytics: true, marketing: true, timestamp: new Date().toISOString() });
            if (cookieBanner) cookieBanner.style.display = 'none';
        });
    }
    ...
    if (btnDecline) {
        btnDecline.addEventListener('click', () => {
            setConsent({ consent: false, analytics: false, marketing: false, timestamp: new Date().toISOString() });
            if (cookieBanner) cookieBanner.style.display = 'none';
        });
    }
    ...
    if (btnSave) {
        btnSave.addEventListener('click', () => {
            const analytics = chkAnalytics ? chkAnalytics.checked : true;
            const marketing = chkMarketing ? chkMarketing.checked : true;
            setConsent({ consent: true, analytics, marketing, timestamp: new Date().toISOString() });
            if (cookieBanner) cookieBanner.style.display = 'none';
        });
    }
